...
fn disassemble(addr: u32, cpu: &Cpu, abus: &mut ABus) {
    ...
    print!("${0:02X}:{1:04X} {2:02X}", addr >> 16, addr & 0xFFFF, opcode);
    let unique_strs: (String, String, String) = match opmode {
        ...
        AddrMode::SrcDst => {
            ( str_operand16!(),
              format!("{0} #${1:02X},#${2:02X}", opname, operand16 >> 8, operand16 & 0xFF),
              String::new() )
        }
        AddrMode::Stack => {
            ( str_operand8!(),
    ...
    print!(" {0:<8} {1:<13} {2:<10}", unique_strs.0, unique_strs.1, unique_strs.2);
    println!(" A:{0:04X} X:{1:04X} Y:{2:04X} {3}", cpu.a(), cpu.x(), cpu.y(), status_str(cpu));
}
